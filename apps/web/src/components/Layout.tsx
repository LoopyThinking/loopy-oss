    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 z-40 flex">
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50" onClick={() => setSidebar(false)} />
          <aside className="relative flex flex-col w-64 bg-white border-r border-gray-100 z-50">
            <button className="absolute top-3 right-3 p-1.5 text-gray-400 hover:text-gray-600" onClick={() => setSidebar(false)}>
              <X size={18} />
            </button>
            {sidebarContent}
          </aside>
      ...
      <div className="flex-1 md:ml-56 flex flex-col min-h-screen">
        ...
        <header className="h-14 bg-white border-b border-gray-100 flex items-center px-4 md:px-6 gap-4 sticky top-0 z-10">
          <button className="md:hidden p-1.5 text-gray-500 hover:text-gray-800" onClick={() => setSidebar(true)}>
            <Menu size={20} />
          </button>
            ...
            {breadcrumbs && breadcrumbs.length > 0
              ? breadcrumbs.map((crumb, i) => (
                  <span key={i} className="flex items-center gap-1.5">
                    ...
                    {crumb.href
                      ? <NavLink to={crumb.href} className="hover:text-gray-800 truncate">{crumb.label}</NavLink>
                      : <span className={i === breadcrumbs.length - 1 ? 'text-gray-800 font-medium truncate' : 'truncate'}>{crumb.label}</span>
                    }
                  </span>
                ))
              : title
          ...
          {/* Org switcher — only show if >1 org or always to show current */}
          {currentOrg && (
            <div className="relative" data-dropdown>
              ...
              >
                ...
                {orgs.length > 1 && <ChevronDown size={13} className="text-gray-400" />}
              </button>
              {orgMenuOpen && orgs.length > 1 && (
                <div className="absolute right-0 mt-1 w-56 bg-white border border-gray-100 rounded-xl shadow-lg py-1 z-50">
                  <p className="px-3 py-1.5 text-xs text-gray-400 font-medium uppercase tracking-wide">Mis organizaciones</p>
                  {orgs.map(org => (
                    <button
                      key={org.id}
          ...
          <div className="relative" data-dropdown>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 mt-1 w-48 bg-white border border-gray-100 rounded-xl shadow-lg py-1 z-50">
                <NavLink to="/settings" className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setUserMenu(false)}>
                  <User size={14} className="text-gray-400" /> Mi perfil
                </NavLink>
                <NavLink to="/settings" className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setUserMenu(false)}>
                  <Key size={14} className="text-gray-400" /> Mi token de agente
                </NavLink>
                <hr className="my-1 border-gray-100" />
                <button onClick={handleSignOut} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-red-600 hover:bg-red-50">
